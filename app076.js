...
function start(){
 ...
 $('#round').textContent=`AI ${diffs[S.diff].label} · LANDSCAPE`;$('#timer').textContent='3';hud();draw(performance.now());
...
function step(f){
 ...
 if(S.lastFoot===f){
  S.combo=0;S.speed*=.42;me.vel*=.35;S.stumbleUntil=now+850;me.stumble=S.stumbleUntil;me.hurtStart=now;S.lastFoot=null;S.shake=10;burst(me,16,true);msg('발 꼬임! 😵',830);next(null);
  setTimeout(()=>{if(S.phase==='run'){next('L');msg('다시! ⚡',380)}},850);return;
 }
 S.lastFoot=f;S.combo++;S.speed=Math.min(10.9,S.speed+1.24+Math.min(1.8,S.combo*.055)*.26);me.vel=Math.max(me.vel,S.speed);me.kick=now;next(f==='L'?'R':'L');burst(me,S.speed>8?5:3,false);
...
function physics(dt,now){
 ...
 S.runners.forEach((r,i)=>{if(!r.finish&&r.dist>=LEN){r.dist=LEN;r.finish=now;if(i===0){S.phase='coast';S.flash=1;S.finishPulse=1;S.shake=7;$('#timer').textContent=((now-S.go)/1000).toFixed(2)+'s';msg('FINISH! 🏁',800);next(null);celebrate();S._res=setTimeout(result,2200)}}});
 if(S.phase==='coast'&&S.runners.every(r=>r.finish))result();
 for(const p of S.dust){p.life-=dt;p.x+=p.vx*dt;p.vy+=25*dt;p.y+=p.vy*dt}
 S.dust=S.dust.filter(p=>p.life>0);
 for(const p of S.confetti){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=18*dt;p.rot+=p.vr*dt}
...
function draw(now){
 const speed=S.runners[0]?.vel||0,tilt=(S.phase==='run'?Math.sin(now*.002)*Math.min(.003,speed*.00025):0);
 ...
 particles();speedLines(now);bottomFade();ctx.restore();
 if(S.flash>0){ctx.fillStyle=`rgba(255,255,255,${S.flash*.38})`;ctx.fillRect(0,0,W,H)}
}
function sky(now){
 const g=ctx.createLinearGradient(0,0,0,190);g.addColorStop(0,'#36bdf7');g.addColorStop(.62,'#8bdcfb');g.addColorStop(1,'#dff8ff');ctx.fillStyle=g;ctx.fillRect(0,0,W,200);
 ctx.fillStyle='rgba(255,255,255,.82)';for(let i=0;i<5;i++){const x=((i*310-now*.004)%1650)-160,y=45+(i%2)*52;ctx.beginPath();ctx.ellipse(x,y,48,14,0,0,7);ctx.ellipse(x+36,y+4,34,12,0,0,7);ctx.ellipse(x-28,y+5,30,10,0,0,7);ctx.fill()}
 ctx.fillStyle='#fff7a8';ctx.globalAlpha=.5;ctx.beginPath();ctx.arc(1090,58,44,0,7);ctx.fill();ctx.globalAlpha=1;
}
function mountains(){
 const shift=cam()*.055;ctx.fillStyle='#67b6ae';ctx.beginPath();ctx.moveTo(0,190);for(let x=-150;x<W+200;x+=130){const y=145+Math.sin((x+shift)*.008)*27;ctx.lineTo(x,y)}ctx.lineTo(W,235);ctx.lineTo(0,235);ctx.fill();
 ctx.fillStyle='#3e9d90';ctx.beginPath();ctx.moveTo(0,210);for(let x=-150;x<W+200;x+=95){const y=185+Math.sin((x+shift*.6)*.013)*17;ctx.lineTo(x,y)}ctx.lineTo(W,245);ctx.lineTo(0,245);ctx.fill();
}
function stadium(now){
 ctx.fillStyle='#253a5b';ctx.fillRect(0,180,W,120);ctx.fillStyle='#314c70';ctx.fillRect(0,185,W,14);
 for(let row=0;row<4;row++)for(let x=4;x<W;x+=16){ctx.fillStyle=['#ffd84d','#ff6c6a','#5ee6ad','#fff','#8ea7ff'][(x/16+row|0)%5];ctx.beginPath();ctx.arc(x+(row%2)*5,215+row*22+Math.sin(x*.06+now*.004+row)*3,3.6,0,7);ctx.fill()}
 ctx.fillStyle='#0f1c31';rr(W/2-190,198,380,54,13);ctx.fill();ctx.strokeStyle='#ffffff30';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle='#fff';ctx.font='900 24px system-ui';ctx.textAlign='center';ctx.fillText('NEMO SPORTS · 50M',W/2,233);
 for(let i=0;i<7;i++){const x=68+i*188;ctx.strokeStyle='#e7eff9';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x,145);ctx.lineTo(x,198);ctx.stroke();ctx.fillStyle=['#ffd84d','#ff5e6d','#5ee6ad','#62c9ff'][i%4];ctx.beginPath();ctx.moveTo(x,146);ctx.quadraticCurveTo(x+31,151+Math.sin(now*.007+i)*6,x+62,148);ctx.lineTo(x+62,171);ctx.quadraticCurveTo(x+31,166+Math.sin(now*.007+i)*6,x,170);ctx.closePath();ctx.fill()}
 ctx.fillStyle='#f1d8ab';ctx.fillRect(0,294,W,9);
}
function track(){
 const top=303,bottom=650;const g=ctx.createLinearGradient(0,top,0,bottom);g.addColorStop(0,'#dc7151');g.addColorStop(1,'#ae4635');ctx.fillStyle=g;ctx.fillRect(0,top,W,bottom-top);
 const laneYs=[315,390,465,540,615];for(let i=0;i<laneYs.length;i++){ctx.strokeStyle='rgba(255,255,255,.72)';ctx.lineWidth=i===0||i===laneYs.length-1?4:2.5;ctx.beginPath();ctx.moveTo(0,laneYs[i]);ctx.lineTo(W,laneYs[i]);ctx.stroke()}
 for(let m=0;m<=50;m+=5){const x=wx(m);if(x<-50||x>W+50)continue;ctx.strokeStyle='rgba(255,255,255,.16)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(x,top);ctx.lineTo(x,bottom);ctx.stroke();ctx.fillStyle='rgba(255,255,255,.72)';ctx.font='800 13px system-ui';ctx.textAlign='left';ctx.fillText(m+'m',x+6,322)}
 for(let i=0;i<22;i++){const x=((i*74-cam()*.45)%1550)-110;ctx.strokeStyle='#ffffff16';ctx.beginPath();ctx.moveTo(x,top);ctx.lineTo(x-62,bottom);ctx.stroke()}
 ctx.fillStyle='#318955';ctx.fillRect(0,bottom,W,8);
}
function finishLine(now){
 const x=wx(50);if(x<-32||x>W+40)return;for(let y=303;y<650;y+=22){ctx.fillStyle=((y/22)|0)%2?'#fff':'#151923';ctx.fillRect(x-9,y,18,22)}
 ctx.fillStyle='#0c1729';rr(x-58,266,116,31,10);ctx.fill();ctx.fillStyle='#fff';ctx.font='900 16px system-ui';ctx.textAlign='center';ctx.fillText('FINISH',x,287);
 if(S.finishPulse>0){ctx.strokeStyle=`rgba(255,216,77,${S.finishPulse})`;ctx.lineWidth=7;ctx.beginPath();ctx.arc(x,486,68+58*(1-S.finishPulse),0,7);ctx.stroke()}
}
function runner(r,now,me){
 const x=wx(r.dist),running=['run','coast'].includes(S.phase)&&!r.finish,hurt=(r===S.runners[0]?now<S.stumbleUntil:now<r.stumble),bounce=running&&!hurt?Math.abs(Math.sin(r.phase))*5:0;
 ctx.save();ctx.translate(x,r.y+bounce);if(me){ctx.fillStyle='rgba(255,216,77,.14)';ctx.beginPath();ctx.arc(0,5,56+Math.sin(now*.01)*3,0,7);ctx.fill()}
 if(r.char.type==='sugar')sugar(r,now,me,running,hurt);else simple(r,now,me,running,hurt);
 ctx.fillStyle=me?'#ffd84d':'rgba(255,255,255,.94)';rr(-34,-72,68,19,9);ctx.fill();ctx.fillStyle='#101928';ctx.font='900 10px system-ui';ctx.textAlign='center';ctx.fillText(me?'1P · 나':r.name,0,-58);ctx.restore();
}
function sugar(r,now,me,running,hurt){
 const s=me?.42:.36,bw=220*s,bh=220*s,stride=running&&!hurt?Math.sin(r.phase):0,ang=hurt?fall(r,now):Math.min(.11,r.vel/78),hit=Math.max(0,1-(now-(r.kick||0))/120);
 ctx.save();ctx.rotate(ang);ctx.fillStyle='rgba(0,0,0,.22)';ctx.beginPath();ctx.ellipse(-5,bh*.5+8,bw*.42,7,0,0,7);ctx.fill();
 const armS=s*.66,legS=s*.62,sy=-bh*.03,sx=bw*.44,hy=bh*.35,hx=bw*.18;let al=-stride*.78,ar=stride*.78,ll=stride*.7,lr=-stride*.7;if(hurt){al=-1.05;ar=.92;ll=.18;lr=-.22}
 ...
 ctx.save();ctx.scale(1+hit*.028,1-hit*.036);pimg(A.body,0,0,s);ctx.restore();
 if(hurt){ctx.strokeStyle='#20191a';ctx.lineWidth=3.5;for(const q of [-1,1]){const ex=q*bw*.15,ey=-bh*.05;ctx.beginPath();ctx.moveTo(ex-4,ey-4);ctx.lineTo(ex+4,ey+4);ctx.moveTo(ex+4,ey-4);ctx.lineTo(ex-4,ey+4);ctx.stroke()}ctx.fillStyle='#b6282d';ctx.beginPath();ctx.ellipse(0,bh*.14,7,5,0,0,7);ctx.fill()}
 else{pimg(A.eyes,0,-bh*.05,s*.54);pimg(A.mouth,0,bh*.12,s*.48);ctx.fillStyle='#ff7d88aa';ctx.beginPath();ctx.ellipse(-bw*.27,bh*.08,4,2.7,0,0,7);ctx.ellipse(bw*.27,bh*.08,4,2.7,0,0,7);ctx.fill()}ctx.restore();
}
function simple(r,now,me,running,hurt){
 const c=r.char,s=me?1.05:.92,w=66*s,h=(c.type==='apt'?88:62)*s,stride=running&&!hurt?Math.sin(r.phase):0,ang=hurt?fall(r,now):Math.min(.09,r.vel/86);
 ctx.save();ctx.rotate(ang);ctx.fillStyle='rgba(0,0,0,.21)';ctx.beginPath();ctx.ellipse(0,h*.54,w*.5,6,0,0,7);ctx.fill();ctx.strokeStyle=c.type==='safe'?'#8d96a1':c.type==='brown'?'#683820':c.type==='brick'?'#973b2e':'#e7d8c2';ctx.lineWidth=9;ctx.lineCap='round';
 for(const [sx,sgn] of [[-1,-1],[1,1]]){ctx.beginPath();ctx.moveTo(sx*w*.22,h*.34);ctx.lineTo(sx*w*.22+sgn*stride*7,h*.52);ctx.stroke();ctx.beginPath();ctx.moveTo(sx*w*.5,0);ctx.lineTo(sx*w*.5-sgn*stride*8,8);ctx.stroke()}
 ctx.fillStyle=c.c;rr(-w/2,-h/2,w,h,13);ctx.fill();detail(c,w,h);face(hurt,w,h);ctx.restore();
}
function detail(c,w,h){
 if(c.type==='brick'){ctx.strokeStyle='#8d3428';ctx.lineWidth=2;for(let y=-h*.25;y<h*.3;y+=14){ctx.beginPath();ctx.moveTo(-w/2+5,y);ctx.lineTo(w/2-5,y);ctx.stroke()}}
 else if(c.type==='safe'){ctx.strokeStyle='#2d333b';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,7,13,0,7);ctx.stroke();ctx.fillStyle='#d0a451';ctx.beginPath();ctx.arc(0,7,4,0,7);ctx.fill()}
 else if(c.type==='apt'){ctx.fillStyle='#62b9e7';for(let y=-h*.3;y<h*.15;y+=17)for(let x=-w*.23;x<w*.28;x+=16)ctx.fillRect(x-3,y,7,8);ctx.fillStyle='#315f61';ctx.fillRect(-7,h*.17,14,14)}
}
function face(hurt,w,h){
 if(hurt){ctx.strokeStyle='#17191d';ctx.lineWidth=3;for(const x of [-11,11]){ctx.beginPath();ctx.moveTo(x-4,-7);ctx.lineTo(x+4,1);ctx.moveTo(x+4,-7);ctx.lineTo(x-4,1);ctx.stroke()}return}
 for(const x of [-11,11]){ctx.fillStyle='#111';ctx.beginPath();ctx.ellipse(x,-5,5.2,7,0,0,7);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(x-1.5,-7,1.6,0,7);ctx.fill()}ctx.fillStyle='#391617';ctx.beginPath();ctx.arc(0,8,7,0,Math.PI);ctx.fill();
}
