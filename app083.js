...
function motionStyle(r,running,hurt){
 if(!running||hurt)return{x:0,y:0,rot:0,stride:1,squash:0};
 const p=r.phase,c=r.char.type;
 if(c==='sugar')return{x:Math.sin(p*.5)*.7,y:-Math.abs(Math.sin(p))*8.5,rot:Math.sin(p)*.025,stride:1.08,squash:Math.max(0,Math.cos(p*2))*.035};
 if(c==='brown')return{x:Math.sin(p*.46)*.5,y:-Math.abs(Math.sin(p*.92))*4.2,rot:Math.sin(p*.75)*.02,stride:.90,squash:Math.max(0,Math.cos(p*1.84))*.02};
 if(c==='brick')return{x:0,y:-Math.abs(Math.sin(p*.78))*2.2,rot:Math.sin(p*.78)*.012,stride:.72,squash:Math.max(0,Math.cos(p*1.56))*.018};
 if(c==='safe')return{x:Math.sin(p*.68)*1.1,y:-Math.abs(Math.sin(p*.7))*1.6,rot:Math.sin(p*.68)*.045,stride:.64,squash:.006};
 if(c==='apt')return{x:Math.sin(p*.48)*2.4,y:-Math.abs(Math.sin(p*.62))*2.0,rot:Math.sin(p*.48)*.075,stride:.56,squash:.004};
 return{x:0,y:-Math.abs(Math.sin(p))*4,rot:0,stride:1,squash:0};
}
function drawMotionCue(r,m,now){
 if(!['run','coast'].includes(S.phase)||r.finish||r.stumble>now)return;
 const c=r.char.type;
 if(c==='brick'&&Math.abs(Math.cos(r.phase*.78))>.94){ctx.globalAlpha=.22;ctx.fillStyle='#fff';ctx.fillRect(-20,35,40,3);ctx.globalAlpha=1}
 if(c==='safe'&&Math.abs(Math.sin(r.phase*.68))>.92){ctx.globalAlpha=.22;ctx.strokeStyle='#fff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,5,43,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1}
}
...
function runner(r,now,me){
 const x=wx(r.dist),running=['run','coast'].includes(S.phase)&&!r.finish,hurt=(r===S.runners[0]?now<S.stumbleUntil:now<r.stumble),m=motionStyle(r,running,hurt);
 ctx.save();ctx.translate(x+m.x,r.y+m.y);
 if(me){ctx.fillStyle='rgba(255,216,77,.14)';ctx.beginPath();ctx.arc(0,5,56+Math.sin(now*.01)*3,0,7);ctx.fill()}
 ctx.save();ctx.rotate(m.rot);ctx.scale(1+m.squash,1-m.squash*.7);drawMotionCue(r,m,now);if(r.char.type==='sugar')sugar(r,now,me,running,hurt,m);else simple(r,now,me,running,hurt,m);ctx.restore();
 ctx.fillStyle=me?'#ffd84d':'rgba(255,255,255,.94)';rr(-34,-72,68,19,9);ctx.fill();ctx.fillStyle='#101928';ctx.font='900 10px system-ui';ctx.textAlign='center';ctx.fillText(me?'1P · 나':r.name,0,-58);if(r.reactUntil>now)reactionBubble(r,now);ctx.restore()
}
