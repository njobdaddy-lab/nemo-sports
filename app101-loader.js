...
function replaceBlock(src,name,nextName,code){
 ...
 if(start<0||end<0){console.warn('[NEMO FLAVOR] optional block missing:',name);return src;}
...
(async()=>{
 ...
 if(c==='sugar')return{x:Math.sin(p*.52)*1.2,y:-Math.abs(Math.sin(p))*14.5,rot:Math.sin(p)*.048,stride:1.24,squash:Math.max(0,Math.cos(p*2))*.068};
 ...
 ctx.save();
 if(c==='sugar'){
  if(hit){ctx.globalAlpha=.38;ctx.strokeStyle='#fff6d8';ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(-5,34,30,7,0,0,Math.PI*2);ctx.stroke();}
  if(Math.abs(Math.sin(p))<.12){ctx.globalAlpha=.25;ctx.fillStyle='#fff';ctx.font='900 14px system-ui';ctx.textAlign='center';ctx.fillText('통!',0,49);}
 }else if(c==='brown'){
