...
const banner=document.createElement('div');banner.className='finalSprintBanner';banner.innerHTML='<strong>🔥 FINAL SPRINT</strong><span>마지막 20m</span><em id="finalRemain">20.0m</em>';stage.appendChild(banner);
...
function enterFinal(m){active=true;tenMeterCalled=false;game.classList.add('finalSprintMode');showBanner(1400);showCallout('🔥 남은 20m! 여기서부터 승부!',900);if(navigator.vibrate)navigator.vibrate(18);lastRank=rank();lastMeters=m;updateRemain(m)}
...
function poll(){
 ...
 if(visible&&m>=30&&m<50){
  if(!active)enterFinal(m);updateRemain(m);
  if(!tenMeterCalled&&m>=40){tenMeterCalled=true;showBanner(900);showCallout('🔥 10m 남았다! 끝까지!',760);if(navigator.vibrate)navigator.vibrate(12)}
  ...
  if(m-lastMeters>2.2&&r!==null){if(r===1&&m>42)showCallout('😤 선두 지켜!');else if(r>1&&m>42)showCallout('💨 잡아! '+r+'위');lastMeters=m}
